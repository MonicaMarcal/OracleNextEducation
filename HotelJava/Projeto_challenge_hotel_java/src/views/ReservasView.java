...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		lblCheckIn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (lblCheckIn.getText().equals("dd/mm/aaaa")) {
					lblCheckIn.setForeground(Color.BLACK);
					lblCheckIn.setText("");
				}
				if (lblCheckOut.getText().isEmpty()) {
					lblCheckOut.setForeground(SystemColor.activeCaptionBorder);
					lblCheckOut.setText("dd/MM/aaaa");
				}

				if (lblValor.getText().isEmpty()) {
					lblValor.setForeground(SystemColor.activeCaptionBorder);
	...
	public void salvarReserva(){
		...
		Reservas reserva = new Reservas(java.sql.Date.valueOf(dataEntrada), java.sql.Date.valueOf(dataSaida), txtValor.getText(), txtFormaPagamento.getSelectedItem().toString());
